...
1. Ensure it is git cloned into `../sources` (relative to the repo root) and uses the exact version that you need for the task in a format like <name>-<version/hash>. To update cloned repositories to the latest commit, run: `../sync.sh <name>` or `../sync.sh --all`.
